...
int main()
{
    ...
    {
        cout << setfill(' ') << setw(29) << " System Deadlocked." << endl
             << endl;

        cout << setfill(' ') << setw(18) << "PROCESS" << setw(8) << "TIME" << endl;

        for (int i = 0; i < deadlock.size(); i++)
            cout << setfill(' ') << setw(15) << deadlock[i].get_name() << setw(10) << deadlock[i].get_time();
    }
